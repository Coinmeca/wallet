...
export default function EthSignTransaction() {
    ...
    const handleSign = async () => {
        setLevel(1);
        const signRequest = provider?.sign(
            {
                to: tx?.to,
                data: tx?.data,
        ...
        if (!signRequest) {
            const error = "Transaction signature request could not be started.";
            reject(error);
            setError(error);
            setLevel(3);
            return;
        ...
        await signRequest
            .then((result) => {
                ...
                if (!result) throw new Error("Transaction signature did not persist.");
                                                ...
                                                <Layouts.Col align={"center"} style={{ padding: "4em" }}>
                                                    <Layouts.Col gap={4} align={"center"} fit>
                                                        ...
                                                        <Elements.Text weight={"bold"} opacity={0.6}>
                                                            {t("request.transaction.sign.complete", {
                                                                to: txToShort,
                                                                origin,
                                                            })}
                                                        </Elements.Text>
