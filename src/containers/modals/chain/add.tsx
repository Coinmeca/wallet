...
const ChainAddModal = (props: any) => {
    ...
    const handleSave = (e: any) => {
        ...
        if (!Object.values(error).some((err: any) => err?.state || Object.values(err)?.some((err: any) => err?.state))) {
            ...
            provider?.addEthereumChain(nextChain);
            props?.onClose(e);
        }
    };
    ...
    return (
        <Modal
            {...props}
            title={
                chain?.chainName?.length
                    ? t("modal.chain.add.title.withName", { chainName: chain?.chainName })
                    : t("modal.chain.add.title")
            }
            onClose={handleClose}
            buttonArea={
                <Layouts.Row>
                                                ...
                                                onClick={() =>
                                                    setChain((state: any) => {
                                                        if (state?.rpcUrls?.length > 1)
                                                            return { ...state, rpcUrls: state?.rpcUrls?.filter((_: any, idx: number) => i !== idx) };
                                                        else {
                                                            setError((error: any) => ({
                                                                ...error,
                                        ...
                                        onClick={() =>
                                            setChain((state: any) =>
                                                state?.blockExplorerUrls?.length && state?.blockExplorerUrls?.length > 1
                                                    ? { ...state, blockExplorerUrls: state?.blockExplorerUrls?.filter((_: any, idx: number) => i !== idx) }
