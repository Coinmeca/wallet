...
export class CoinmecaWalletProvider extends CoinmecaWalletBase {
    ...
    #data(_?: { key?: string; storage?: CloudStorage | Storage }) {
        ...
        return loadStorage(this.codename, _?.storage || user ? telegram?.CloudStorage : localStorage, _?.storage ? false : isTelegram, _?.key);
    }
    ...
    get address(): string {
        return this.#data({ storage: sessionStorage })?.get("account") || this.#data()?.get("account");
    }
    ...
    get chainId() {
        const id = this.#data({ storage: sessionStorage })?.get("chain") || this.#data()?.get("chain");
        return id ? formatChainId(id) : undefined;
    }

    get chain() {
        return this.chains?.find((c: Chain) => c?.chainId === this.chainId);
    }
    ...
    changeAccount(index: number | string) {
        ...
        return this.#safe(() => {
            ...
            this.#data({ storage: sessionStorage }).set("account", account?.address);
    ...
    changeChain(chainId: number | string) {
        ...
        if (chains?.length && chains?.find((c: Chain) => c?.chainId === chainId)) {
            ...
            this.#data({ storage: sessionStorage }).set("chain", chainId);
    ...
    addFungibleAsset(address: string) {
        ...
        if (account?.tokens?.fungibles) {
            const tokens = account.tokens.fungibles?.[`${this.chainId}`];
            if (Array.isArray(tokens)) {
                const exist = tokens?.map((a) => a?.toLowerCase() === address?.toLowerCase());
                if (!exist) {
                    account.tokens.fungibles[`${this.chainId}`] = [...tokens, address];
                    this.#storage?.set(account.address?.toLowerCase(), account);
                    this.emit("updateFungibleAsset");
                }
            } else {
                account.tokens.fungibles = { ...account.tokens.fungibles, [`${this.chainId}`]: [address] };
                this.#storage?.set(account.address?.toLowerCase(), account);
                this.emit("updateFungibleAsset");
            }
        } else {
            if (account?.tokens) account.tokens = { ...account.tokens, fungibles: { [`${this.chainId}`]: [address] } };
            else account = { ...account, tokens: { fungibles: { [`${this.chainId}`]: [address] } } };
            this.#storage?.set(account.address?.toLowerCase(), account);
            this.emit("updateFungibleAsset");
        }
    }
    ...
    removeFungibleAsset(address: string) {
        ...
        if (account?.tokens?.fungibles) {
            ...
            if (Array.isArray(tokens)) {
                const exist = tokens?.map((a) => a?.toLowerCase() === address?.toLowerCase());
                if (!exist) {
                    account.tokens.fungibles[`${this.chainId}`] = [...tokens?.filter(a => a?.toLowerCase() !== address?.toLowerCase())];
                    this.#storage?.set(account.address?.toLowerCase(), account);
                    this.emit("updateFungibleAsset");
                }
            }
